...
<section>
  ...
  <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" onsubmit="return confirmInsert();">
    <label for="favorite_id">Favorite ID:</label>
    <input type="number" id="favorite_id" name="favorite_id"><br><br>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $cvp = $connection->prepare("INSERT INTO favorites (favorite_id, user_id, poll_id) VALUES (?, ?, ?)");
      $cvp->bind_param("iii", $favorite_id, $user_id, $poll_id);
  ...
  <table border="1">
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    <td>" . $row['favorite_id'] . "</td>
                    <td>" . $row['user_id'] . "</td>
                    <td>" . $row['poll_id'] . "</td>
                    <td><a style='padding:4px' href='delete_favorites.php?favorite_id=$favorite_id'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_favorites.php?favorite_id=$favorite_id'>Update</a></td> 
                </tr>";
        }
    } else {
